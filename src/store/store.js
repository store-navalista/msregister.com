...
export default class Store {
  ...
  async checkAuth() {
    ...
    try {
      const response = await axios.get(
        "https://www.msregister.com:5000/api/refresh",
  ...
  async fetchUsers(page = 1) {
    try {
      ...
      const users = await response.data.map(
        (user, index) => (user.table_id = index + 1)
      );
  ...
  async updatePasswordUser(passwordId, newPassword) {
    try {
      const response = await UserService.updatePasswordUser(
        passwordId,
        newPassword
      );
      return response;
